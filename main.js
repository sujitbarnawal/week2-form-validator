var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
var form = document.getElementById('validation-form');
var usernameInput = document.getElementById('username');
var emailInput = document.getElementById('email');
var passwordInput = document.getElementById('password');
var ageInput = document.getElementById('age');
var countrySelect = document.getElementById('country');
function displayError(input, message) {
    var _a;
    removeError(input);
    var error = document.createElement("div");
    error.classList = "error-message";
    error.textContent = message;
    (_a = input.parentElement) === null || _a === void 0 ? void 0 : _a.appendChild(error);
}
function removeError(input) {
    var parent = input.parentElement;
    var error = parent === null || parent === void 0 ? void 0 : parent.querySelector(".error-message");
    if (error)
        error.remove();
}
function ValidateUsername(username) {
    var regex = /^[a-zA-Z0-9]{3,20}$/;
    if (!regex.test(username)) {
        return { isValid: false, error: "Username must be 3-20 alphanumeric characters" };
    }
    return { isValid: true };
}
function ValidatePassword(password) {
    var regex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/;
    if (!regex.test(password)) {
        return { isValid: false, error: "Password does not meet requirements" };
    }
    return { isValid: true };
}
function ValidateAge(age) {
    if (age < 18 || age > 120) {
        return { isValid: false, error: "Age must be between 18 to 120" };
    }
    return { isValid: true };
}
function ValidateEmail(email) {
    return __awaiter(this, void 0, void 0, function () {
        var error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, new Promise(function (res, rej) { return setTimeout(res, 700); })];
                case 1:
                    _a.sent();
                    if (email.endsWith("@test.com")) {
                        throw new Error("This email domain is not allowed");
                    }
                    return [2 /*return*/, { isValid: true }];
                case 2:
                    error_1 = _a.sent();
                    if (error_1 instanceof Error) {
                        return [2 /*return*/, { isValid: false, error: error_1.message }];
                    }
                    return [2 /*return*/, { isValid: false, error: "Unknown email validation error" }];
                case 3: return [2 /*return*/];
            }
        });
    });
}
form.addEventListener("submit", function (e) { return __awaiter(_this, void 0, void 0, function () {
    var formState, isFormValid, usernameResult, passwordResult, ageResult, emailResult;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                e.preventDefault();
                removeAllErrors();
                formState = {
                    username: usernameInput.value.trim(),
                    email: emailInput.value.trim(),
                    password: passwordInput.value,
                    age: Number(ageInput.value),
                    country: countrySelect.value
                };
                isFormValid = true;
                usernameResult = ValidateUsername(formState.username);
                if (!usernameResult.isValid) {
                    displayError(usernameInput, usernameResult.error);
                    isFormValid = false;
                }
                passwordResult = ValidatePassword(formState.password);
                if (!passwordResult.isValid) {
                    displayError(passwordInput, passwordResult.error);
                    isFormValid = false;
                }
                ageResult = ValidateAge(formState.age);
                if (!ageResult.isValid) {
                    displayError(ageInput, ageResult.error);
                    isFormValid = false;
                }
                if (!formState.country) {
                    displayError(countrySelect, "Please select a country");
                    isFormValid = false;
                }
                return [4 /*yield*/, ValidateEmail(formState.email)];
            case 1:
                emailResult = _a.sent();
                if (!emailResult.isValid) {
                    displayError(emailInput, emailResult.error);
                    isFormValid = false;
                }
                if (!isFormValid)
                    return [2 /*return*/];
                alert("Form Validated Successfully!");
                form.reset();
                return [2 /*return*/];
        }
    });
}); });
function removeAllErrors() {
    var errors = document.querySelectorAll('.error-message');
    errors.forEach(function (error) { return error.remove(); });
}
